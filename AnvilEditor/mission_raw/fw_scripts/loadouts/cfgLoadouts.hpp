class CfgRespawnInventory
{
    class FWLoadoutAAR
    {
        displayName = "Asst. Autorifleman";
        icon = "\A3\Ui_f\data\GUI\Cfg\Hints\rifle_ca.paa";
        weapons[] = {
            "arifle_MXC_F",
            "launch_Titan_F",
            "Binocular",
            "NVGoggles",
            "hgun_Pistol_heavy_01_F"
        };
        magazines[] = {
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "100Rnd_65x39_caseless_mag",
            "100Rnd_65x39_caseless_mag",
            "100Rnd_65x39_caseless_mag",
            "100Rnd_65x39_caseless_mag",
            "100Rnd_65x39_caseless_mag",
            "DemoCharge_Remote_Mag",
            "DemoCharge_Remote_Mag",
            "Titan_AA",
            "11Rnd_45ACP_Mag",
            "11Rnd_45ACP_Mag",
            "SmokeShell",
            "SmokeShell",
            "HandGrenade",
            "HandGrenade",
            "HandGrenade"
        };
        items[] = {
            "FirstAidKit",
            "FirstAidKit"
        };
        linkedItems[] = {
            "V_PlateCarrierGL_rgr",
            "H_HelmetB_grass",
            "optic_Aco",
            "acc_flashlight",
            "muzzle_snds_acp",
            "muzzle_snds_H",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_CombatUniform_mcam_tshirt";
        backpack = "B_TacticalPack_mcamo";
    };


    class FWLoadoutAMG
    {
        displayName = "Asst. Machinegunner";
        icon = "\A3\Ui_f\data\GUI\Cfg\Hints\rifle_ca.paa";
        weapons[] = {
            "arifle_MXC_F",
            "Binocular",
            "NVGoggles",
            "hgun_Pistol_heavy_01_F"
        };
        magazines[] = {
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "200Rnd_65x39_cased_Box",
            "200Rnd_65x39_cased_Box",
            "200Rnd_65x39_cased_Box_Tracer",
            "200Rnd_65x39_cased_Box_Tracer",
            "DemoCharge_Remote_Mag",
            "11Rnd_45ACP_Mag",
            "11Rnd_45ACP_Mag",
            "HandGrenade",
            "HandGrenade",
            "HandGrenade"
        };
        items[] = {
            "FirstAidKit",
            "FirstAidKit"
        };
        linkedItems[] = {
            "V_PlateCarrierGL_rgr",
            "H_HelmetB_grass",
            "optic_Aco",
            "acc_flashlight",
            "muzzle_snds_acp",
            "muzzle_snds_H",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_CombatUniform_mcam_tshirt";
        backpack = "B_TacticalPack_mcamo";
    };
    
    
    class FWLoadoutAT
    {
        displayName = "AT Rifleman";
        icon = "\A3\Ui_f\data\GUI\Cfg\Hints\launcher_ca.paa";
        weapons[] = {
            "arifle_MXC_F",
            "launch_B_Titan_short_F",
            "Binocular",
            "NVGoggles",
            "hgun_Pistol_heavy_01_F"
        };
        magazines[] = {
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "11Rnd_45ACP_Mag",
            "11Rnd_45ACP_Mag",
            "SmokeShell",
            "SmokeShell",
            "HandGrenade",
            "HandGrenade",
            "HandGrenade",
            "Titan_AT",
            "Titan_AT",
            "Titan_AT"
        };
        items[] = {
            "FirstAidKit",
            "FirstAidKit"
        };
        linkedItems[] = {
            "V_PlateCarrierGL_rgr",
            "H_HelmetB_grass",
            "optic_Aco",
            "acc_flashlight",
            "muzzle_snds_acp",
            "muzzle_snds_H",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_CombatUniform_mcam_tshirt";
        backpack = "B_TacticalPack_mcamo";
    };
    
    
    class FWLoadoutAR
    {
        displayName = "Autorifleman";
        icon = "\A3\Ui_f\data\GUI\Cfg\Hints\tracers_ca.paa";
        weapons[] = {
            "arifle_MX_SW_F",
            "Binocular",
            "NVGoggles",
            "hgun_Pistol_heavy_01_F"
        };
        magazines[] = {
            "100Rnd_65x39_caseless_mag_Tracer",
            "100Rnd_65x39_caseless_mag_Tracer",
            "100Rnd_65x39_caseless_mag_Tracer",
            "100Rnd_65x39_caseless_mag_Tracer",
            "100Rnd_65x39_caseless_mag",
            "100Rnd_65x39_caseless_mag",
            "100Rnd_65x39_caseless_mag",
            "100Rnd_65x39_caseless_mag",
            "11Rnd_45ACP_Mag",
            "11Rnd_45ACP_Mag",
            "SmokeShell",
            "SmokeShell",
            "HandGrenade",
            "HandGrenade",
            "HandGrenade"
        };
        items[] = {
            "FirstAidKit",
            "FirstAidKit"
        };
        linkedItems[] = {
            "V_PlateCarrierGL_rgr",
            "H_HelmetB_grass",
            "optic_Aco",
            "acc_flashlight",
            "muzzle_snds_acp",
            "muzzle_snds_H",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_CombatUniform_mcam_tshirt";
        backpack = "B_TacticalPack_mcamo";
    };
    
    
    class FWLoadoutGL
    {
        displayName = "Grenadier";
        icon = "\A3\Ui_f\data\GUI\Cfg\Hints\grenades_ca.paa";
        weapons[] = {
            "arifle_MX_GL_F",
            "Binocular",
            "NVGoggles",
            "hgun_Pistol_heavy_01_F"
        };
        magazines[] = {
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "11Rnd_45ACP_Mag",
            "11Rnd_45ACP_Mag",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "3Rnd_SmokeGreen_Grenade_shell",
            "3Rnd_SmokeRed_Grenade_shell",
            "3Rnd_SmokeBlue_Grenade_shell",
            "SmokeShell",
            "SmokeShell",
            "HandGrenade",
            "HandGrenade",
            "HandGrenade"
        };
        items[] = {
            "FirstAidKit",
            "FirstAidKit"
        };
        linkedItems[] = {
            "V_PlateCarrierGL_rgr",
            "H_HelmetB_grass",
            "optic_Aco",
            "acc_flashlight",
            "muzzle_snds_acp",
            "muzzle_snds_H",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_CombatUniform_mcam_tshirt";
        backpack = "B_TacticalPack_mcamo";
    };


    class FWLoadoutMG
    {
        displayName = "Machinegunner";
        icon = "\A3\Ui_f\data\GUI\Cfg\Hints\tracers_ca.paa";
        weapons[] = {
            "LMG_Mk200_F",
            "Binocular",
            "NVGoggles",
            "hgun_Pistol_heavy_01_F"
        };
        magazines[] = {
            "200Rnd_65x39_cased_Box_Tracer",
            "200Rnd_65x39_cased_Box_Tracer",
            "200Rnd_65x39_cased_Box_Tracer",
            "200Rnd_65x39_cased_Box",
            "200Rnd_65x39_cased_Box",
            "200Rnd_65x39_cased_Box",
            "11Rnd_45ACP_Mag",
            "11Rnd_45ACP_Mag",
            "SmokeShell",
            "SmokeShell",
            "HandGrenade",
            "HandGrenade"
        };
        items[] = {
            "FirstAidKit",
            "FirstAidKit"
        };
        linkedItems[] = {
            "V_PlateCarrierGL_rgr",
            "H_HelmetB_grass",
            "optic_MRCO",
            "acc_flashlight",
            "muzzle_snds_acp",
            "muzzle_snds_H_MG",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_CombatUniform_mcam_tshirt";
        backpack = "B_TacticalPack_mcamo";
    };
    
    class FWLoadoutSL
    {
        displayName = "Squad Leader";
        icon = "\A3\Ui_f\data\GUI\Cfg\Ranks\lieutenant_gs.paa";
        weapons[] = {
            "arifle_MX_GL_F",
            "Binocular",
            "NVGoggles",
            "hgun_Pistol_heavy_01_F"
        };
        magazines[] = {
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "1Rnd_HE_Grenade_shell",
            "3Rnd_UGL_FlareWhite_F",
            "3Rnd_UGL_FlareGreen_F",
            "3Rnd_UGL_FlareRed_F",
            "11Rnd_45ACP_Mag",
            "11Rnd_45ACP_Mag",
            "SmokeShellGreen",
            "SmokeShellRed",
            "SmokeShell",
            "SmokeShell",
            "HandGrenade",
            "HandGrenade",
            "HandGrenade"
        };
        items[] = {
            "FirstAidKit",
            "FirstAidKit"
        };
        linkedItems[] = {
            "V_PlateCarrierGL_rgr",
            "H_HelmetB_grass",
            "optic_MRCO",
            "acc_flashlight",
            "muzzle_snds_acp",
            "muzzle_snds_H",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_CombatUniform_mcam_tshirt";
        backpack = "B_TacticalPack_mcamo";
    };

    class FWLoadoutTL
    {
        displayName = "Team Leader";
        icon = "\A3\Ui_f\data\GUI\Cfg\Ranks\sergeant_gs.paa";
        weapons[] = {
            "arifle_MX_F",
            "Binocular",
            "NVGoggles",
            "hgun_Pistol_heavy_01_F"
        };
        magazines[] = {
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "11Rnd_45ACP_Mag",
            "11Rnd_45ACP_Mag",
            "SmokeShellGreen",
            "SmokeShellGreen",
            "SmokeShellRed",
            "SmokeShellRed",
            "SmokeShell",
            "SmokeShell",
            "SmokeShell",
            "SmokeShell",
            "SmokeShell",
            "SmokeShell",
            "HandGrenade",
            "HandGrenade",
            "HandGrenade"
        };
        items[] = {
            "FirstAidKit",
            "FirstAidKit"
        };
        linkedItems[] = {
            "V_PlateCarrierGL_rgr",
            "H_HelmetB_grass",
            "optic_MRCO",
            "acc_flashlight",
            "muzzle_snds_acp",
            "muzzle_snds_H",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_CombatUniform_mcam_tshirt";
        backpack = "B_TacticalPack_mcamo";
    };
    
    class FWLoadoutCW
    {
        displayName = "Crewman";
        icon = "\A3\Ui_f\data\GUI\Cfg\Ranks\sergeant_gs.paa";
        weapons[] = {
            "SMG_01_F",
            "Binocular",
            "NVGoggles_INDEP",
            "hgun_P07_F"
        };
        magazines[] = {
            "30Rnd_45ACP_Mag_SMG_01",
            "30Rnd_45ACP_Mag_SMG_01",
            "16Rnd_9x21_Mag",
            "16Rnd_9x21_Mag",
            "FlareRed_F",
            "FlareGreen_F",
            "SmokeShell",
            "SmokeShellGreen",
            "APERSMine_Range_Mag",
            "APERSMine_Range_Mag"
        };
        items[] = {
            "FirstAidKit"
        };
        linkedItems[] = {
            "H_HelmetCrew_B",
            "G_Tactical_Clear",
            "H_HelmetB_grass",
            "optic_Holosight_smg",
            "acc_flashlight",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_CombatUniform_mcam";
    };
    
    class FWLoadoutPLT
    {
        displayName = "Pilot";
        icon = "\A3\Ui_f\data\GUI\Cfg\Ranks\sergeant_gs.paa";
        weapons[] = {
            "SMG_01_F",
            "Binocular",
            "NVGoggles_OPFOR",
            "hgun_P07_F"
        };
        magazines[] = {
            "30Rnd_45ACP_Mag_SMG_01",
            "30Rnd_45ACP_Mag_SMG_01",
            "16Rnd_9x21_Mag",
            "16Rnd_9x21_Mag",
            "FlareRed_F",
            "FlareGreen_F",
            "SmokeShell",
            "SmokeShellGreen",
            "SmokeShellRed"
        };
        items[] = {
            "FirstAidKit"
        };
        linkedItems[] = {
            "U_B_PilotCoveralls",
            "G_Tactical_Clear",
            "H_PilotHelmetHeli_B",
            "optic_Holosight_smg",
            "acc_flashlight",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_PilotCoveralls";
    };

    class FWLoadoutWS
    {
        displayName = "Weapons Specialist";
        icon = "\A3\Ui_f\data\GUI\Cfg\Ranks\private_gs.paa";
        weapons[] = {
            "arifle_MX_F",
            "Binocular",
            "NVGoggles",
            "hgun_Pistol_heavy_01_F"
        };
        magazines[] = {
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "11Rnd_45ACP_Mag",
            "11Rnd_45ACP_Mag",
            "SmokeShell",
            "SmokeShell",
            "SmokeShell",
            "HandGrenade",
            "HandGrenade",
            "HandGrenade"
        };
        items[] = {
            "FirstAidKit",
            "FirstAidKit"
        };
        linkedItems[] = {
            "V_PlateCarrierGL_rgr",
            "H_HelmetB_grass",
            "optic_Aco",
            "acc_flashlight",
            "muzzle_snds_acp",
            "muzzle_snds_H",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_CombatUniform_mcam_tshirt";
        backpack = "B_TacticalPack_mcamo";
    };

    class FWLoadoutOFF
    {
        displayName = "Officer";
        icon = "\A3\Ui_f\data\GUI\Cfg\Ranks\captain_gs.paa";
        weapons[] = {
            "arifle_MXC_F",
            "Laserdesignator",
            "NVGoggles",
            "hgun_Pistol_heavy_01_F"
        };
        magazines[] = {
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "11Rnd_45ACP_Mag",
            "11Rnd_45ACP_Mag",
            "SmokeShell",
            "SmokeShell",
            "SmokeShell",
            "SmokeShell",
            "SmokeShellRed",
            "SmokeShellRed",
            "SmokeShellGreen",
            "SmokeShellGreen",
            "SmokeShellOrange",
            "SmokeShellBlue",
            "HandGrenade",
            "HandGrenade",
			"Chemlight_green",
			"Chemlight_green"
        };
        items[] = {
            "FirstAidKit",
            "FirstAidKit"
        };
        linkedItems[] = {
            "V_PlateCarrierH_CTRG",
            "H_MilCap_mcamo",
			"G_Tactical_Clear",
            "optic_Arco",
			"optic_MRD",
            "acc_flashlight",
            "muzzle_snds_acp",
            "muzzle_snds_H",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_CombatUniform_mcam";
        backpack = "B_FieldPack_cbr";
    };

    class FWLoadoutMD
    {
        displayName = "Medic";
        icon = "\A3\Ui_f\data\GUI\Cfg\Ranks\corporal_gs.paa";
        weapons[] = {
            "arifle_MXC_F",
            "Binocular",
            "NVGoggles",
            "hgun_Pistol_heavy_01_F"
        };
        magazines[] = {
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "30Rnd_65x39_caseless_mag_Tracer",
            "11Rnd_45ACP_Mag",
            "11Rnd_45ACP_Mag",
            "SmokeShell",
            "SmokeShell",
            "SmokeShell",
            "SmokeShell",
            "SmokeShell",
            "SmokeShell"
        };
        items[] = {
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
            "FirstAidKit",
			"Medikit"
        };
        linkedItems[] = {
            "V_PlateCarrierL_CTRG",
            "H_HelmetB_grass",
            "optic_Aco",
			"optic_MRD",
            "acc_flashlight",
            "muzzle_snds_acp",
            "muzzle_snds_H",
            "ItemMap",
            "ItemCompass",
            "ItemWatch",
            "ItemRadio",
            "ItemGPS"
        };
        uniformClass = "U_B_CombatUniform_mcam";
        backpack = "B_FieldPack_cbr";
    };

};